	...
	const addJob = useCallback(
		async (job: JobApplication) => {
			...
			try {
				...
				// Ensure we update the state in a safe way that will trigger re-renders
				if (isMountedRef.current) {
					safeSetJobs((prevJobs) => [completeJob, ...prevJobs])
					console.log('Job added to state:', completeJob)
				}
